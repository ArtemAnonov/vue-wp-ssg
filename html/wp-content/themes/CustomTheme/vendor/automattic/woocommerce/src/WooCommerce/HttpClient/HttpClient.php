...
class HttpClient
{
    ...
    protected function setDefaultCurlSettings()
    {
        ...
        //     // Proxy for Docker
        //     curl_setopt($this->ch, CURLOPT_PROXY, $_SERVER['SERVER_ADDR'] . ':' .  $_SERVER['SERVER_PORT']);
        // }
    ...
    public function request($endpoint, $method, $data = [], $parameters = [])
    {
        ...
        if (\curl_errno($this->ch)) {
            throw new HttpClientException('cURL Error: ' . \curl_error($this->ch) . 'curl_errno :' . \curl_errno($this->ch), 0, $request, $response);
        }
